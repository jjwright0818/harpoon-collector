...
SELECT 
  ...
  ARRAY_AGG(ROUND(size, 2) ORDER BY size DESC) as size_amounts,
...
SELECT 
  ...
  MIN(size) as smallest_trade_usd,
  MAX(size) as largest_trade_usd,
  ROUND(AVG(size), 2) as average_trade_usd,
  CASE 
...
SELECT 
  ...
  trader_username,
  COUNT(*) as trade_count,
  ROUND(SUM(size), 2) as total_usd_spent,
  ROUND(AVG(size), 2) as avg_trade_usd,
